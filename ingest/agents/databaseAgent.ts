...
const saveToDatabaseTool = createTool({
  ...
  parameters: z.object({
    fileDisplayName: z
      .string()
      .describe(
        "The readable display name of the receipt to show on the UI. If the file name is not human readable, use this to give more readable name.",
      ),
    receiptId: z.string().describe("The ID of the receipt to update"),
    merchantName: z.string(),
    merchantAdress: z.string(),
  ...
  handler: async (params, { step, network }) => {
    ...
    const result = await step?.run("save-receipt-to-database", async () => {
      try {
        //Call convex mutation to update the receipt with extracted data
    ...
    if (result?.addedToDb === "Success") {
      ...
      network?.state.kv.set("receipt", receiptId);
    }
...
export const databaseAgent = createAgent({
  ...
  system:
    "You are helpfull assistant that takes key information regarding receipts and saves it to the conves database",
  model: openai({
    model: "gpt-4o-mini",
    defaultParameters: {
