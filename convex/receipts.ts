...
export const generateUploadUrl = mutation({
  args: {},
  handler: async (ctx) => {
    //Generate a Url taht the client can use to upload a file
    return await ctx.storage.generateUploadUrl();
  },
});

//Store a receipt file and add it to the DB
export const storeReceipt = mutation({
  ...
  handler: async (ctx, args) => {
    const receiptId = await ctx.db.insert("receipts", {
      ...
      status: "pending",
      //Initialize extracted data fileds as null
      merchantName: undefined,
      merchantAdress: undefined,
...
export const getReceiptDownloadUrl = query({
  ...
  handler: async (ctx, args) => {
    //Get a temp URL that can be used to download file
...
export const updateReceiptStatus = mutation({
  ...
  handler: async (ctx, args) => {
    //Verify user has acces to this receipt
    const receipt = await ctx.db.get(args.id);
    if (!receipt) {
...
export const updateReceiptWithExtractedData = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(args.id, {
      ...
      merchantAdress: args.merchantAdress,
